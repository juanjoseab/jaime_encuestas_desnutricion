...
class reportesController extends Display {
    ...
    function deploy() {
        ...
        $anios = new MysqlSelect();
        $anios->setTableReference("submision");
        $anios->setDistinct();
        $anios->addSelection("submision", "anio");
        $anios->execute();
        $this->data = array();
        $this->data["anios"] = $anios->rows;
    ...
    function visualizar() {
        ...
        $val->rows;
        $this->grid .= " ['Mes','Total',";
        $this->gridPorcentaje = " ['Mes','100 Por ciento',";
        $custmSel = "";
        ...
        $sl->addSelection("submision", "mes", "mes");
        $sl->addSelection("submision", "anio", "anio");
        ...
        $sl->addGroup("submision", "mes");

        $sl->addGroup("indicador", "indicador_id");
        $sl->addOrderBy("submision", "anio", "ASC");
        $sl->execute();
        //echo $sl->query; die; 
        ...
        if (count($sl->rows) > 0) {
            foreach ($sl->rows as $row) {
                if (is_array($row)) {
                    ...
                    $this->gridPorcentaje .= " ['" . $this->covertirAMes($row['mes']) . " - " . $row['anio'] . "',100, ";
